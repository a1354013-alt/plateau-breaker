...
from fastapi import HTTPException # Standardized import
...
def update_record(
    ...
    if "record_date" in update_data and update_data["record_date"] != record.record_date:
        ...
        if existing_record_on_new_date:
            raise HTTPException(status_code=409, detail=f"Record for date {update_data["record_date"]} already exists for another record.")
...
def get_records_by_days(session: Session, days: int) -> list[HealthRecord]:

    all_records = get_all_records_ordered(session)
    if not all_records:
        return []
