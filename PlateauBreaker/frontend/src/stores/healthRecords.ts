...
export const useHealthRecordsStore = defineStore('healthRecords', () => {
  ...
  async function fetchRecords(params?: {
    ...
    // Update current query params if new ones are provided, otherwise use existing ones
    if (params) {
      currentQueryParams.value = { ...currentQueryParams.value, ...params }
    }
    
    try {
      const res = await healthRecordsApi.list(currentQueryParams.value)
