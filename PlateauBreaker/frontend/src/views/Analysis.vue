<template>
  <div class="page-container">
    ...
    <!-- Loading state -->
    <div v-if="loading && !analyticsStore.summaryStatus" class="loading-state">
      <ProgressSpinner style="width:48px;height:48px" />
      <p>Analysing your data...</p>
    </div>

    <template v-else-if="analyticsStore.summaryStatus !== 'insufficient_data'">
      <!-- Plateau Status Card -->
      ...
      <div class="grid-3" style="margin-bottom: 1.5rem;">
        ...
        <div class="card metric-card">
          ...
          <div class="metric-sub" v-if="analyticsStore.plateauData?.avg_change != null">
            Change: <span :class="analyticsStore.plateauData.avg_change < 0 ? 'text-success' : analyticsStore.plateauData.avg_change > 0 ? 'text-danger' : ''">
        ...
        <div class="card metric-card">
          ...
          <div class="metric-sub">
            <span :class="(analyticsStore.plateauData?.last7_fluctuation ?? 1) <= 0.6 ? 'text-warning' : 'text-success'">
              {{ (analyticsStore.plateauData?.last7_fluctuation ?? 1) <= 0.6 ? 'Plateau range (≤±0.3kg)' : 'Active fluctuation' }}
            </span>
          </div>
        </div>
      </div>
      ...
      <div class="card rules-card" style="margin-bottom: 1.5rem;">
        ...
        <div class="rules-grid">
          <div class="rule-item" :class="analyticsStore.plateauData?.rule_a === true ? 'rule-triggered' : analyticsStore.plateauData?.rule_a === false ? 'rule-clear' : 'rule-na'">
            <div class="rule-icon">
              <i :class="analyticsStore.plateauData?.rule_a === true ? 'pi pi-exclamation-circle' : analyticsStore.plateauData?.rule_a === false ? 'pi pi-check-circle' : 'pi pi-minus-circle'" />
            </div>
          ...
          <div class="rule-item" :class="analyticsStore.plateauData?.rule_b === true ? 'rule-triggered' : analyticsStore.plateauData?.rule_b === false ? 'rule-clear' : 'rule-na'">
            <div class="rule-icon">
              <i :class="analyticsStore.plateauData?.rule_b === true ? 'pi pi-exclamation-circle' : analyticsStore.plateauData?.rule_b === false ? 'pi pi-check-circle'" />
            </div>
            <div class="rule-content">
